---
name: pkg-common-util-coding-standards-and-best-practices
description: 'Use after carecard-must-do. Mandatory for every pkg-common-util task, including analysis, clarification, planning, implementation, review, debugging, documentation, public API work, skill maintenance, and validation. Use before every narrower skill.'
---

# Pkg Common Util Coding Standards And Best Practices
...
After `carecard-must-do`, load this skill before doing any work in `pkg-common-util`, including read-only
...
- `$pkg-common-util-api-contract-library` for response, error, middleware,
  export, type, and validation contracts; and
...
1. Read the complete request and inspect implementation, CommonJS and ESM
   exports, declarations, tests, documentation, and consumers before deciding
...
2. Translate the request into a coherent technical contract. Do not apply
   ...
   incompatible with response or package architecture.
...
4. Ask for clarification when an unresolved choice would materially change a
   public API, response or error contract, security, consumer behavior,
   destructive scope, or the repositories that must change.
5. Explain architectural tradeoffs before a major API, type, middleware,
   module, package, or dependency change.
...
- Preserve CommonJS, ESM, declaration, response, error, middleware, naming, and
  test conventions unless the task explicitly replaces them.
...
- Prefer readable direct implementation over clever compression or a temporary
  workaround.
- Use meaningful names that describe API and middleware intent.
...
- Cover the new function, public surfaces, types, and every migrated consumer
  through TDD.
...
migrated and repository-native search proves them unused. Runtime, ESM, and
type tests verify the supported public behavior and consumer contract without
inspecting implementation structure. This does not authorize unrelated API
removal. If an existing published or security contract requires compatibility
and the request does not clearly supersede it, explain the conflict and ask
first.

## TDD And Root-Cause Gate

Follow `$carecard-workspace-standards` and
`$pkg-common-util-api-contract-library` for the complete failing-test-first and
...
- Keep validation, transformation, response construction, middleware control,
  and error mapping responsibilities explicit.
...
2. Run focused runtime, ESM, and type tests, then all broader checks required by
   local skills.
